# Spec: Parallel Compare

## Purpose

The parallel-compare capability governs how `--compare` runs are executed when the CLI is given two servers to evaluate side by side. Instead of running each server's suite sequentially, both suites are launched concurrently so neither server waits for the other to start or finish. Output is withheld until both runs complete, and file naming conventions are preserved exactly as in the sequential case.
