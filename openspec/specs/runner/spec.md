...
The runner SHALL spawn the MCP server file using `StdioServerParameters` with `sys.executable` as the command and the server path as an argument, establishing a real stdio transport.

#### Scenario: Server file not found raises FileNotFoundError

- **WHEN** `suite["server"]` refers to a path that does not exist on disk
- **THEN** `run_suite()` raises `FileNotFoundError` with a message containing the server path

#### Scenario: Server spawns and initializes successfully

- **WHEN** the server file exists
...
When the API response has `stop_reason == "tool_use"`, the runner SHALL dispatch each `tool_use` block to `session.call_tool` and append the tool result to the message history before the next iteration.

#### Scenario: Tool calls are recorded in the trace

- **WHEN** the model issues a tool call during a task
- **THEN** the trace list gains an entry `{"tool": <name>, "params": <input>}` for each dispatched call
