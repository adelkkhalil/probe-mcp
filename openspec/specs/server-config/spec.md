...
The server-config module defines how MCP servers are declared, located, and resolved. Named server entries are stored in a `servers.yaml` file that the loader finds via a two-step lookup and parses into a command/args/cwd dict consumed by the runner.
